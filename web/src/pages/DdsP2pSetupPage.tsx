...
export function DdsP2pSetupPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [grpRes, stdRes, softRes, roleRes, assignRes] = await Promise.all([
  ...
  const matrixRows = useMemo(() => {
    ...
    return out
  }, [kpiGroups, stdByGroup, softByGroup])

  /** Same roster role rows and ordering as Plan 24 (`activeRoles` on Plan24Page). */
  const activeRoles = useMemo(
    () => rosterRoleRows.filter((r) => r.is_active).sort((a, b) => a.sort_order - b.sort_order),
    [rosterRoleRows],
  )

  /** Same persisted prefs as Plan 24 day grid (`mergeViewPrefs` / `loadViewPrefs`). */
  useEffect(() => {
    if (!cellId) {
  ...
  /** Matches Plan 24 visible columns (`roleColsView` on Plan24Page). */
  ...
  async function toggleCell(roleId: string, row: Extract<MatrixRow, { rowKind: 'question' }>, next: boolean) {
    ...
    if (next) {
      ...
      let q = supabase.from('dds_p2p_cell_question_role_assignments').delete().eq('master_cell_id', cellId).eq('roster_role_id', roleId)
      if (row.source === 'standard') {
  ...
  return (
    <div className={ddsStack}>
      <p className="max-w-2xl text-xs leading-snug text-muted">
        ...
        shown in <span className="font-bold text-fg">bold</span>; cell soft-point questions are normal weight. Manage
        questions under{' '}
      ...
      <section className={ddsSection}>
        ...
        <p className="mt-0.5 text-[11px] leading-snug text-muted">
          ...
          sort order, minus roles you have hidden under Plan 24 view preferences.
        </p>
