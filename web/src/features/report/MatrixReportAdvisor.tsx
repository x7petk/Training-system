...
function AdvisorBarChart(props: { spec: ReportAdvisorChartSpec }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-surface p-3">
      <p className="mb-2 text-sm font-semibold text-fg">{spec.title}</p>
          ...
          <div className="flex h-[min(80vh,720px)] w-[min(96vw,80vw)] max-w-5xl flex-col overflow-hidden rounded-2xl border border-border bg-surface-raised shadow-2xl">
            <header className="flex shrink-0 items-start justify-between gap-3 border-b border-border bg-gradient-to-r from-sky-500/12 via-transparent to-indigo-500/10 px-4 py-3 sm:px-5">
              ...
              <button
                ...
                className="shrink-0 rounded-lg border border-border bg-surface px-2.5 py-1.5 text-sm font-medium text-muted hover:bg-black/[0.05] hover:text-fg"
            ...
            <div ref={listRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-3 sm:px-5">
              {messages.length === 0 && !allowSend ? (
                <p className="text-sm text-amber-800">Loading report…</p>
              ) : null}
              {messages.length === 0 && allowSend && !supabaseConfigured ? (
              ...
              {invokeError ? (
                <p className="rounded-lg border border-amber-500/50 bg-amber-50 px-3 py-2 text-sm text-amber-950">{invokeError}</p>
              ) : null}
            </div>
            ...
            <footer className="shrink-0 border-t border-border bg-surface/90 px-3 py-3 sm:px-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
                <textarea
                  ...
                  placeholder={allowSend ? 'Your question' : 'Loading report…'}
                  aria-label="Your question"
                  className="min-w-0 flex-1 resize-none rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-fg outline-none ring-accent/25 focus:border-accent/40 focus:ring-2 disabled:opacity-50"
                  onKeyDown={(e) => {
