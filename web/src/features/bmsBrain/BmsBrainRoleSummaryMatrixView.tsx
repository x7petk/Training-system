...
import { BmsBrainRoleSummaryCell } from './BmsBrainRoleSummaryCell'
...
type Props = {
  ...
  zoom: number
  cells: Map<string, RoleSummaryCell>
  stepPresence: Map<string, boolean>
  loading: boolean
  error: string | null
}

export function BmsBrainRoleSummaryMatrixView({
  roles,
  forums,
  filters,
  viewportWidth,
  ...
  const summaryCellMinH = Math.max(cellMinH, Math.round(110 * blockScale))
    ...
    <div className="space-y-2">
      {loading ? (
        <div className="flex items-center gap-2 text-xs text-muted">
          ...
          Generating role × forum summaries…
        </div>
      ) : null}
