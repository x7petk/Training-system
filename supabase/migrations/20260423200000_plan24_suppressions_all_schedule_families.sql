-- Cross-role drag writes plan24_check_schedule_occurrence_suppressions so materialize skips
-- the vacated (schedule_id, occurrence, role) tuple. FK to plan24_check_schedules only
-- broke CL / CIL / Quality moves. Drop FK (UUID is still unique per schedule row) and
-- teach family materializers to honor the same suppression rows.

alter table public.plan24_check_schedule_occurrence_suppressions
  drop constraint if exists plan24_check_schedule_occurrence_suppressions_schedule_id_fkey;
