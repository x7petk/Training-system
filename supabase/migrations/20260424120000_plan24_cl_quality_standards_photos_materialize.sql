-- CL + Quality template tasks: standard text, reference photo, optional target (nominal).
-- Materialized sub_tasks include operator fields (entered_value, result, text_value) initialized null.

alter table public.plan24_cl_check_template_tasks
  add column if not exists standard_description text,
  add column if not exists photo_path text,
  add column if not exists target_value numeric;

alter table public.plan24_quality_check_template_tasks
  add column if not exists standard_description text,
  add column if not exists photo_path text,
  add column if not exists target_value numeric;
