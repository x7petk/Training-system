-- CIL route templates: per-task recurrence, standard text, photo path, check-type tags.
-- Defects: link to CIL template + task + optional master area/equipment UUIDs.
-- Events: denormalized cil_template_id for defect history when schedule link is cleared.
-- Storage: public bucket for task photos (upload restricted to admins).
...
alter table public.plan24_cil_check_template_tasks
  ...
  add column if not exists recurrence_kind text not null default 'daily'
    check (recurrence_kind in ('hourly', 'daily', 'weekly', 'monthly')),
  ...
  add column if not exists check_types text[] not null default '{}'::text[];

alter table public.plan24_cil_check_template_tasks
  drop constraint if exists plan24_cil_check_template_tasks_interval_ck;

alter table public.plan24_cil_check_template_tasks
  add constraint plan24_cil_check_template_tasks_interval_ck check (interval_n > 0);

alter table public.plan24_cil_check_template_tasks
  drop constraint if exists plan24_cil_check_template_tasks_month_day_ck;

alter table public.plan24_cil_check_template_tasks
  add constraint plan24_cil_check_template_tasks_month_day_ck
    check (month_day is null or (month_day >= 1 and month_day <= 31));

alter table public.plan24_cil_check_template_tasks
  drop constraint if exists plan24_cil_check_template_tasks_check_types_ck;

alter table public.plan24_cil_check_template_tasks
  add constraint plan24_cil_check_template_tasks_check_types_ck
    check (check_types <@ array['cleaning', 'inspection', 'lubrication']::text[]);
...
alter table public.dh_defects
  ...
  add column if not exists cil_template_id uuid references public.plan24_cil_check_templates (id) on delete set null,
  add column if not exists cil_template_task_id uuid references public.plan24_cil_check_template_tasks (id) on delete set null;

create index if not exists dh_defects_cil_template_idx
  on public.dh_defects (master_cell_id, cil_template_id)
  where deleted_at is null and cil_template_id is not null;

alter table public.plan24_events
  add column if not exists cil_template_id uuid references public.plan24_cil_check_templates (id) on delete set null;

create index if not exists plan24_events_cil_template_idx
  on public.plan24_events (master_cell_id, cil_template_id)
  where deleted_at is null and cil_template_id is not null;

insert into storage.buckets (id, name, public)
values ('cil-task-photos', 'cil-task-photos', true)
on conflict (id) do update set public = excluded.public;

drop policy if exists "cil_task_photos_select_auth" on storage.objects;
create policy "cil_task_photos_select_auth"
  on storage.objects for select to authenticated
  using (bucket_id = 'cil-task-photos');

drop policy if exists "cil_task_photos_write_admin" on storage.objects;
create policy "cil_task_photos_write_admin"
  on storage.objects for all to authenticated
  using (bucket_id = 'cil-task-photos' and public.is_app_admin())
  with check (bucket_id = 'cil-task-photos' and public.is_app_admin());

-- ---------------------------------------------------------------------------
-- CIL materialize: per-task recurrence filter + richer sub_tasks + cil_template_id on events
-- ---------------------------------------------------------------------------
...
set cil_template_id = sch.template_id
from public.plan24_cil_check_schedules sch
where e.schedule_id = sch.id
  and e.event_type = 'cil_check'
  and e.cil_template_id is null;
