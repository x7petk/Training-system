-- PL/pgSQL: `FOR s IN SELECT s.* FROM ... s` makes `s` in `s.*` ambiguous (loop variable vs table alias).
-- Use a distinct range-table alias (`sch`) in the driver query; loop body still uses `s` as the row.
