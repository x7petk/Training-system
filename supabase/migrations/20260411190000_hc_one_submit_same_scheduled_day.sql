-- Duplicate guard aligned to "scheduled day":
-- block only when same completer + same type + same cell + same scheduled day.
-- For roster-linked rows we use ldr_assignments.assignment_date.
-- For non-roster rows fallback to completed_at UTC date.
